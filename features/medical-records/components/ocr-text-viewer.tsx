...
import { Edit3, Save, X, FileText, Loader2, AlertCircle, CheckCircle } from "lucide-react"
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge className={getStatusColor()}>
              {getStatusIcon()}
              <span className="ml-1">{record.ocrStatus.replace("_", " ")}</span>
            </Badge>
        ...
        {record.ocrProcessedAt && (
          <p className="text-xs text-gray-500">Processed on {new Date(record.ocrProcessedAt).toLocaleString()}</p>
        )}
      </CardHeader>
      ...
      <CardContent>
        {record.ocrStatus === "PENDING" || record.ocrStatus === "PROCESSING" ? (
          <div className="flex items-center justify-center py-8 text-gray-500">
            ...
            <span>
              {record.ocrStatus === "PENDING" ? "Waiting to process document..." : "Extracting text from document..."}
            </span>
          </div>
